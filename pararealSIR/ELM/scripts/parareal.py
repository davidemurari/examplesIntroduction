...
def getCoarse(time,data,previous=[],networks=[]):
    ...
    if len(previous)==0:
        for i in range(len(time)-1):
            flow = flowMap(y0=coarse_approx[i],initial_proj=initial_proj,weight=weight,bias=bias,dt=dts[i],n_t=n_t,n_x=n_x,L=L,LB=LB,UB=UB,act_name="Tanh")
        ...
        for i in range(len(time)-1):
            flow = flowMap(y0=previous[i],initial_proj=initial_proj,weight=weight,bias=bias,dt=dts[i],n_t=n_t,n_x=n_x,L=L,LB=LB,UB=UB,act_name="Tanh")
            if len(networks)>0:
...
def getNextCoarse(time,y,i,data,networks=[]):
    ...
    flow = flowMap(y0=y,initial_proj=initial_proj,weight=weight,bias=bias,dt=dts[i],n_t=n_t,n_x=n_x,L=L,LB=LB,UB=UB,act_name="Tanh")
    if len(networks)>0:
        flow.computed_projection_matrices = networks[i].computed_projection_matrices.copy()
        flow.y0 = y
    flow.approximate_flow_map()
